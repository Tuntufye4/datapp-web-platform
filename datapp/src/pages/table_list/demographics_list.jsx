...
export default function DemographicsTablePage() {                     
  ...
  // Export to CSV
  const exportCSV = () => {
    const headers = ["ID","Age","Sex","District","Supervising facility","Address"];
    const csvRows = [
      headers.join(","), // header row
      ...filteredRows.map(r =>
        [r.id, r.age, r.sex, r.district, r.supervising_facility, r.address].join(",")
      )
    ];
        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-gray-50">
              <tr>
                {["ID","Age", "Sex","District","Supervising facility","Address"].map(h => (
            ...
            <tbody>
              {filteredRows.map((r) => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-3 py-2">{r.supervising_facility}</td>
